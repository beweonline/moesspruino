...
( function(){
  var p // for temp prototype
    ...
    , flotData = []
    , flotChart
    , flotOptions = {legend:{noColumns:5,container:"#flotlegendholder"},grid:{show: true, hoverable:true},
        xaxis:{mode:"time",ticks:0},yaxes:[{},{min:0,max:1.05,show:false}]
      }
    , flotDatasetOptions = {lines:{ show:true},clickable:false,hoverable:true}
...
/*   function switchLogging(){
  ...
  function showDataPoints(){
    var i,html = "",ds;
    html += '<table border="0" id="detailsTable">';
    if(datapoints.length === 0){
		
      datapoints.push(new Datapoint({label:"a",expression:"a",type:"number"}));
      datapoints.push(new Datapoint({label:"b",expression:"b",type:"number"}));
...
/*       if(testLogging === true){
  ...
  function showFlotCharts(){
    ...
    for(var i = 0; i < datapoints.length;i++){
     ...
     setTimeout(function(){
      if($("#placeholder")[0]){
        flotChart = $.plot($("#placeholder"),flotData,flotOptions);
		$("#placeholder").resize(function () {console.log('resized');});
